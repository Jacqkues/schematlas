    ...
    view! {
        ...
        >
            ...
            <div class="absolute top-[15px] right-[15px] z-10 flex items-center gap-1 rounded-[7px] border border-line-soft bg-surface-3 p-[3px] text-soft shadow-[0_3px_12px_#0004]" on:pointerdown=|ev| ev.stop_propagation()>
                <button type="button" class=TOOL_BUTTON aria-label="Auto layout" title=move || source.with(|s| if s.kind == "openapi" { "Align routes with their models and dependencies" } else { "Arrange tables by domain and relationships" }) disabled=move || arranging.get() on:click=move |_| arrange()>

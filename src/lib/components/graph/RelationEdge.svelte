...
<BaseEdge
  ...
  {path}
  interactionWidth={12}
  class="stroke-edge [stroke-width:1.3] transition-[stroke,stroke-width,opacity] duration-200 [.edge-active_&]:stroke-edge-active [.edge-active_&]:[stroke-width:2] [.edge-muted_&]:stroke-edge [.edge-muted_&]:[stroke-width:1] [.edge-muted_&]:opacity-[0.14] [.selected_&]:stroke-accent [.selected_&]:[stroke-width:2]"
/>
{#if data?.showLabels}
  <g class="pointer-events-auto animate-fade-in" aria-label={String(data.description ?? '')}>
    ...
    {#each [{ x: sourceX + 34, y: sourceY - 12, text: data.sourceCardinality }, { x: targetX - 34, y: targetY - 12, text: data.targetCardinality }] as label}
      {#if label.text}<g transform={`translate(${label.x},${label.y})`}
          ><rect
            class="fill-accent-soft stroke-accent-line [stroke-width:1]"
            x="-24"
            y="-12"
            width="48"
            height="23"

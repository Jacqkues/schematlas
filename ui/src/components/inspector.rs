...
pub fn Inspector(
    ...
    let badge = |text: String| {
        view! { <span class="rounded-[3px] border border-line px-1 py-0.5 font-mono text-[9px] text-text">{text}</span> }
    };
    let title = format!(
        "{}{}",
        entity
    ...
    view! {
        <aside class="inspector w-[300px] shrink-0 overflow-y-auto border-l border-line-soft bg-sidebar max-[1180px]:w-[270px] max-[1000px]:absolute max-[1000px]:inset-y-0 max-[1000px]:right-0 max-[1000px]:z-[8] max-[1000px]:shadow-[-6px_0_20px_#0006]">
            ...
            <div class="border-b border-line-soft px-5 py-[23px]">
                ...
                {entity.description.clone().filter(|d| !d.is_empty()).map(|d| view! {
                    <p class="mt-[13px] text-xs leading-[1.8] whitespace-pre-wrap text-soft [overflow-wrap:anywhere]">{d}</p>
                })}
            </div>
                    ...
                    view! {
                        <div class="border-t border-line-soft px-1 py-3">
                            <div class="flex items-start justify-between gap-3">
                                <strong class="flex gap-1.5 font-mono text-xs font-normal text-ink [overflow-wrap:anywhere]">
                                    {field.primary_key.then(|| view! { <Icon name="key-round" size=12 /> })}
                                    {field.name.clone()}
                                </strong>
                                <code class="max-w-[130px] text-right text-[11px] text-soft [overflow-wrap:anywhere]">{data_type}</code>
                            </div>
                            ...
                            {field.default_value.clone().map(|value| view! {
                                <p class="mt-[7px] text-[11px] leading-relaxed text-soft [overflow-wrap:anywhere]">"Default: " <code>{value}</code></p>
                            })}
                            {field.description.clone().filter(|d| !d.is_empty()).map(|d| view! {
                                <p class="mt-[7px] text-[11px] leading-relaxed text-soft [overflow-wrap:anywhere]">{d}</p>
                            })}
                        </div>
                    }
                }).collect_view()}
                        ...
                        view! {
                            ...
                            >
                                ...
                                <div class="min-w-0 flex-1">
                                    <strong class="block font-mono text-[11px] font-normal [overflow-wrap:anywhere]">{name}</strong>
